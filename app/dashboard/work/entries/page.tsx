...
export default function EntriesPage() {
  ...
  const getSummary = async () => {
    ...
    try {
      ...
      if (snapshot.exists()) {
        ...
        setShowSummary(true)
        setShowUsage(false)
        // Fetch pending orders when showing summary
        fetchPendingOrders()
      }
    } catch (error) {
      toast({
  ...
  const renderMainContent = () => {
    if (showSummary && summaryData.length > 0) {
      return (
        <motion.div
        ...
        >
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-2xl font-semibold">Quantity Summary</h2>
          ...
          {/* Summary Table Card */}
          <Card className={`${theme === 'dark' ? 'bg-gray-800/50' : 'bg-white'} backdrop-blur-md border-0 shadow-lg mb-6`}>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>Mother Entries</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {summaryData.map((item, index) => (
                    <TableRow key={index}>
                      <TableCell>{item.productDestination}</TableCell>
                      <TableCell>{item.remainingQuantity.toFixed(2)}</TableCell>
                      <TableCell>{item.estimatedTrucks.toFixed(2)}</TableCell>
                      <TableCell>
                        {item.motherEntries.map(entry => 
                          `${entry.number} (${entry.remainingQuantity})`
                        ).join(', ')}
                      </TableCell>
                    </TableRow>
                  ))}
